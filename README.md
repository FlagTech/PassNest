...
PassNest 提供兩種方式讓 AI 代理人安全操作。**AI 永遠無法取得密碼明文**，只能查看條目的中繼資料（名稱、網址、帳號），或觸發「自動輸入」讓 PassNest 自己把密碼貼入目前作業系統焦點所在的欄位——密碼字串全程留在 PassNest 本機 server 裡，不會回傳給 AI。
...
|------|---------|
| 查看條目名稱、網址、帳號 | ✅ |
| 觸發自動輸入（貼入目前 OS focus 欄位，僅 Windows） | ✅ |
| 讀取密碼或 API 金鑰明文 | ❌ |
| 新增、修改、刪除條目 | ❌ |
...
| POST | `/autotype` | 將指定欄位貼入目前 OS focus 的控制項（僅 Windows） |
...
先點擊目標網頁的密碼欄位讓它取得真正的作業系統焦點，再呼叫：

```bash
curl -X POST http://localhost:7070/autotype \
  -H "Content-Type: application/json" \
  -d "{\"token\":\"$TOKEN\",\"entryId\":\"<ID>\",\"field\":\"password\",\"expectedWindowTitle\":\"<目標網站網域名>\"}"
# → {"success":true,"data":{"label":"Gmail","field":"password"}}
```

`field` 可用值：`"password"` / `"username"` / `"keyValue"`

`expectedWindowTitle` 為選填但強烈建議帶入——PassNest 會核對目前作業系統前景視窗標題是否包含這個字串，不符合就直接中止並回傳 409，不會送出任何按鍵。密碼只會短暫寫入系統剪貼簿並立即以真實 Ctrl+V 貼上、隨即清空，明文不會出現在任何 HTTP 回應中。目前僅支援 Windows。
...
| 404 | 找不到指定條目或欄位 |
| 409 | 目前前景視窗與 `expectedWindowTitle` 不符，已中止自動輸入 |
| 429 | 自動輸入請求過於頻繁，請稍候再試 |
| 500 | 伺服器內部錯誤，或平台不支援（非 Windows） |
